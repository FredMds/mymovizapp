...
class App extends Component {
  ...
  componentDidMount() {
    var ctx = this;
    fetch('http://localhost:3001/movie').then(function(response) {
      return response.json();
    }).then(function(listeFilm) {
      ctx.setState({movies: listeFilm.results});
    ...
    fetch('http://localhost:3001/mymovies').then(function(response) {
      ...
      ctx.setState({moviesLiked: listeFilm.movies});
    }).catch(function(error) {
      console.log('Request failed', error)
    });
  ...
  handleClick(isLike, name) {
    console.log("Bonjour le batch 8");
    console.log(isLike);
      ...
      if (this.state.movies !== undefined) {
            ...
            for (var i = 0; i < this.state.movies.length; i++) {
              ...
              for (var y = 0; y < this.state.moviesLiked.length; y++) {
                if (this.state.movies[i]._id === this.state.moviesLiked[y].idMovieDB) {
                  isLike = true;
                  break;
                }
              }

              moviesLiked.push(<Movie key={i}
                          handleClickParent={this.handleClick}
                          idMovieDB={this.state.movies[i].id}
                          movieName={this.state.movies[i].title}
                          movieDesc={this.state.movies[i].overview.substr(0, 100) + "..."}
                          movieImage={this.state.movies[i].poster_path}
          ...
          <Nav>
            ...
            <NavItem>
              <NavLink onClick={this.handleClickLikeOff} style={{
                ...
                Les derniers films</NavLink>
            </NavItem>
            <NavItem>
              <NavLink onClick={this.handleClickLikeOn} style={{
                  color: '#FFFFFF'
                }} href="#">Mes favoris</NavLink>
            </NavItem>
            <NavItem>
              <NavLink href="#">
...
class Movie extends Component {
  ...
  handleClick() {
    ...
    if (isLike === true) {
      ...
      fetch('http://localhost:3001/mymovies/' + this.props.id, {method: 'DELETE'})
    }
